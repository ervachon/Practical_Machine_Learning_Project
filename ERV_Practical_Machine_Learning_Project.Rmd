...
**You can access to the html file of this document** [here](http://ervachon.github.io/Practical_Machine_Learning_Project/)
...
```
We can see that the new_window variable is "yes" or "no" so we transforme it in 0 or 1
```{r,message=FALSE, warning=FALSE}
train$new_window <- as.integer(train$new_window=="yes")
test$new_window  <- as.integer(test$new_window=="yes")
```

Now we made a correlation matrix to determine the most correlated feature (min 90%) and drop them. Of course we can do a PCA preprocess but we don't choose to do it.
```{r,message=FALSE, warning=FALSE}
...
###7. Errors and confusion matrix on our test set (20% of the original training set).
...
```
So we have a accuracy of `r round(accuracy*100,2)`% so an error rate of `r round(errorRate*100,2)`% and a sensitivity (mean of each class) of `r round(meanSensitivity*100,2)`%. Now the confusion Matrix in percentage.
```{r,message=FALSE, warning=FALSE}
